...
class TileMap:
    ...
    def raycast_solid(
        ...
        """Bresenham grid raycast.  Returns ``True`` if any solid tile lies between *start* and *end*.

        Both *start* and *end* are world-pixel coordinates.
        """
        ts = self.tile_size
        ...
        while True:
            # Skip the starting tile itself (the entity's own tile)
    ...
    def get_enemy_spawns(self) -> list[dict]:
        ...
        Each entry is a dict with keys ``pos``, ``tier``, and optionally
        ``patrol_path``.
        """
